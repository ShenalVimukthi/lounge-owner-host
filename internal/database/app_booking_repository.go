...
// UpdateBookingStatusByID updates the status of an app booking using the new schema
func (r *AppBookingRepository) UpdateBookingStatusByID(bookingID string, status string, boardedAt *time.Time) error {
	var query string
	var args []interface{}

	if boardedAt != nil {
		query = `UPDATE bus_booking_seats SET status = $1, boarded_at = $2, updated_at = NOW() 
		         WHERE id IN (SELECT id FROM bus_booking_seats WHERE bus_booking_id = $3)`
		args = []interface{}{status, *boardedAt, bookingID}
	} else {
		query = `UPDATE bus_booking_seats SET status = $1, updated_at = NOW() 
		         WHERE id IN (SELECT id FROM bus_booking_seats WHERE bus_booking_id = $3)`
		args = []interface{}{status, bookingID}
	}
...
// VerifyBookingByReferenceAndTrip verifies a booking by reference and scheduled trip ID
func (r *AppBookingRepository) VerifyBookingByReferenceAndTrip(reference string, scheduledTripID string) (*models.MasterBooking, error) {
	query := `
		SELECT b.* 
		FROM bookings b
		INNER JOIN bus_bookings bb ON bb.booking_id = b.id
		WHERE b.booking_reference = $1 AND bb.scheduled_trip_id = $2
		LIMIT 1`

	var booking models.MasterBooking
	err := r.db.Get(&booking, query, reference, scheduledTripID)
	if err != nil {
		return nil, err
	}
